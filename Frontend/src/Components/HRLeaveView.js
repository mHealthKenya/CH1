...
export class HRLeaveView extends Component {
	state = {
		leaveRequests: [],
		leaveRequest: {},
		comments: "",
	...
	componentDidMount = () => {
		...
		if (isAuthenticated) {
			...
			axios.get(`leaveapplicationscoo/?approved=true`).then((res) => {
				const { data } = res;
				this.setState({
					...this.state,
					leaveRequests: data,
...
export default connect(mapStateToProps, mapDispatchToProps)(HRLeaveView);
